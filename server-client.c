...
have_event:
	...
	/* Scroll acceleration. */
	if ((type == KEYC_TYPE_WHEELUP || type == KEYC_TYPE_WHEELDOWN) &&
	    loc == KEYC_MOUSE_LOCATION_PANE && wp != NULL &&
	    c->session != NULL &&
	    options_get_number(c->session->options, "scroll-acceleration")) {
		...
		if (wme == NULL) {
			...
			max_lines = options_get_number(c->session->options,
			    "scroll-acceleration-max-speed");

			scroll_up_new = (MOUSE_BUTTONS(b) == MOUSE_WHEEL_UP);
			...
			if (c->scroll_last_time.tv_sec == 0 &&
				...
				timersub(&now, &c->scroll_last_time, &diff);
			...
			c->scroll_events[c->scroll_event_idx] = now;
			c->scroll_event_idx =
			    (c->scroll_event_idx + 1) % SCROLL_EVENT_BUF;
			...
			else if (c->scroll_fast_accum_ms > delta_ms * 2)
				c->scroll_fast_accum_ms -= delta_ms * 2;
			else
				c->scroll_fast_accum_ms = 0;
			...
			c->scroll_accum %= 1000;
			if (lines > 0)
				window_copy_scroll_one(wp, c->scroll_up, lines);
		}
	}
